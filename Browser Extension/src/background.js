...
async function run(dataUrl) {
  ...
  // For text-and-images input (multimodal), use the gemini-pro-vision model
      ...
      const prompt = "Identify the dark patterns in this  screenshot of a website, and only give the patterns which blatantly try to scam the end consumer, tell where it is located with the keywords involved. Also list the other dark patterns that are not that important under other. Make this into 2 lists, one with the serious dark patterns, the other with the milder ones. Have a threshold on the dark patterns displayed. If it is not significant and there aren't many of them, just say There are no major dark patterns here. Stick to the context of the website, Do not deviate from what's there on the image, under a new heading keywords, mention all the keywords that are there on the provided image that indicate a darkpattern in a serialized form.Do not add numbers in front of the keywords. check for deviations and hallucinations before you give an output";
